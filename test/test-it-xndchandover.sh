...
#
# usage:
# ./test-it-xndchandover.sh
#
# e.g. ./test-it-xndchandover.sh
#
##########################

echo "Running IT Xn Dual Connectivity Handover (Xn DC Handover) test"
...
cd goTest
go test -v -vet=off -run TestXnDCHandover
go_test_exit_code=$?
cd ..
