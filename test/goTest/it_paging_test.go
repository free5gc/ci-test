...
func TestPaging(t *testing.T) {
	...
	// ============================================
	// Trigger downlink data so AMF pages the idle UE.
	//
	// Reference test injects a UDP packet from inside UPF's own "UPFns" network
	// namespace (a bare-metal test topology construct). Our UPF runs as its own
	// Docker container, so that namespace doesn't exist here. Instead: add a route
	// for the UE's DNN subnet via UPF on this container, then send the UDP packet
	// directly from Go. The UE's PDU session IP is dynamically allocated by SMF from
	// the 10.60.0.0/16 pool (not observed here) and hardcoded below to match the
	// reference test's assumption of the first address in the pool; if Paging is not
	// received, check the allocated IP (PDU Address IE in the PDU Session
	// Establishment Accept) against this value and update it.
	// ============================================
	if out, rerr := exec.Command("ip", "route", "add", "10.60.0.0/16", "via", UPF_IP).CombinedOutput(); rerr != nil {
		t.Logf("[DEBUG] add route to UE subnet via UPF (ignored, may already exist): %s: %v", string(out), rerr)
	}

	uePduSessionIp := "10.60.0.1"
	dlConn, err := net.Dial("udp", uePduSessionIp+":8080")
	assert.Nil(t, err)
	_, err = dlConn.Write([]byte("hello"))
	assert.Nil(t, err)
	dlConn.Close()
	...
	require.Nil(t, err)
	assert.Equal(t, ngapPdu.Present, ngapType.NGAPPDUPresentInitiatingMessage, "Not NGAPPDUPresentInitiatingMessage")
	assert.Equal(t, ngapPdu.InitiatingMessage.ProcedureCode.Value, ngapType.ProcedureCodePaging, "Not ProcedureCodePaging")

	ue.AmfUeNgapId = 2

	// send NAS Service Request
	pdu = GetServiceRequest(nasMessage.ServiceTypeMobileTerminatedServices)
