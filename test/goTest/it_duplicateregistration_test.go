...
func TestDuplicateRegistration(t *testing.T) {
	...
	require.Nil(t, err)
	_, err = n2Conn.Write(pduSessionEstablishmentRequestMsg)
	require.Nil(t, err)
	...
	// send PDU Session Establishment Request again to trigger duplicate handling
	_, err = n2Conn.Write(pduSessionEstablishmentRequestMsg)
	require.Nil(t, err)
	...
	// receive NGAP PDU Session Resource Setup Request (second)
	n, err = n2Conn.Read(recvMsg)
	require.Nil(t, err)
	_, err = ngapMessage.Parse(recvMsg[:n])
	require.Nil(t, err)
	...
	// send NGAP PDU Session Resource Setup Response (second)
	sendMsg, err = GetPDUSessionResourceSetupResponse(10, ue.AmfUeNgapId, ue.RanUeNgapId, IT_IP)
	require.Nil(t, err)
	_, err = n2Conn.Write(sendMsg)
	require.Nil(t, err)
