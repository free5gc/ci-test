...
func TestRequestTwoPDUSessions(t *testing.T) {
	...
	// ============================================
	// First PDU Session Establishment (session ID 10, slice SD="010203", dnn="internet")
	// ============================================
	...
	// ============================================
	// Second PDU Session Establishment (session ID 11, slice SD="112233", dnn="internet").
	// Reference test uses dnn="internet2" and a different UE IP/port for a dummy ICMP
	// probe here, but this environment's second slice only has DNN "internet" configured
	// (see smfCfg.yaml), and the dummy probe never validated a reply, so it's dropped.
	// ============================================
	sNssai = models.Snssai{
		Sst: SST,
		Sd:  SD_2,
	}
	pdu = GetUlNasTransport_PduSessionEstablishmentRequest(11, nasMessage.ULNASTransportRequestTypeInitialRequest, "internet", &sNssai)
	pdu, err = EncodeNasPduWithSecurity(ue, pdu, nas.SecurityHeaderTypeIntegrityProtectedAndCiphered, true, false)
	assert.Nil(t, err)
	sendMsg, err = GetUplinkNASTransport(ue.AmfUeNgapId, ue.RanUeNgapId, pdu)
	assert.Nil(t, err)
	...
	// send NGAP PDU Session Resource Setup Response
	sendMsg, err = GetPDUSessionResourceSetupResponse(11, ue.AmfUeNgapId, ue.RanUeNgapId, IT_IP)
	assert.Nil(t, err)
	_, err = n2Conn.Write(sendMsg)
	assert.Nil(t, err)
